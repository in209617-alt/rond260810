...
| `js/net.js` | Firebase 연결 (익명 로그인, 방 입장, 위치 전송·수신) |
| `js/firebase-config.js` | **내 Firebase 프로젝트 값을 넣는 곳** |
| `database.rules.json` | Realtime Database 보안 규칙 (최대 2명, 자기 캐릭터만 수정) |
...
3. Realtime Database의 **규칙** 탭을 열고, `database.rules.json` 내용을 통째로 붙여 넣은 뒤 **게시**.
4. **빌드 → Authentication → 시작하기 → 로그인 방법 → 익명** 을 **사용 설정**.
...
- 혼자 테스트할 때는 **같은 컴퓨터에서 탭 두 개**로 열어도 돼요. 탭마다 다른 플레이어로 들어가요.
...
- 창을 닫거나 인터넷이 끊기면 서버가 자동으로 그 플레이어를 지워요 (`onDisconnect`).
  브라우저가 강제 종료된 경우에는 서버가 끊김을 알아채기까지 최대 1분 정도 캐릭터가 남아 있을 수 있어요.
